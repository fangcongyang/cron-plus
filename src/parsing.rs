...
use nom::sequence::{delimited, separated_pair, terminated, tuple, pair};
use nom::IResult;
...
fn daymonth_last_day_name(i: &str) -> IResult<&str, String> {
    map(
        delimited(multispace0, tag("L"), multispace0),
        ToOwned::to_owned,
    )(i)
}
...
mod test {
    ...
    #[test]
    fn test_nom_valid_last_week_day() {
        let expression = "2W";
        month_last_word_day(expression).unwrap();
    }
